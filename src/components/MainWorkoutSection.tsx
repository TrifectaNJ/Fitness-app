...
interface MainWorkoutSectionProps {
  workoutId: string;
...
const MainWorkoutSection: React.FC<MainWorkoutSectionProps> = ({
  workoutId,
  ...
  useEffect(() => {
    const handleExerciseCompleted = async (event: any) => {
      console.log('Main workout exercise completed:', event.detail);
      if (user && workoutId) {
        setTimeout(async () => {
  ...
  const loadMainWorkoutSubsections = async () => {
    try {
      const { data: subsectionsData, error: subsectionsError } = await supabase
        .from('workout_subsections')
        .select('*')
        .eq('workout_id', workoutId)
        .eq('section_key', 'main-workout')
        .order('sort_order');

      if (subsectionsError) {
        console.error('Error loading main workout subsections:', subsectionsError);
        return;
      }
      ...
      console.error('Error loading main workout subsections:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div>
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-4">
          <h2 className="text-xl font-bold text-gray-900">MAIN WORKOUT</h2>
...
export default MainWorkoutSection;