...
import { Plus, Search, GraduationCap, Users, DollarSign, RefreshCw, Database } from 'lucide-react';
...
const CoachProgramsManagerFixed: React.FC = () => {
  ...
  const loadPrograms = async () => {
    try {
      ...
      // Then get assignments with user profiles
      ...
      // Combine the data
      const programsWithUsers = (programsData || []).map(program => ({
        ...program,
        assigned_users: (assignmentsData || [])
          .filter(assignment => assignment.coach_program_id === program.id)
          .map(assignment => ({
  ...
  const totalRevenue = programs.reduce((sum, program) => sum + (program.price || 0), 0);
  ...
  const getDbStatusText = () => {
    switch (dbStatus) {
      ...
      case 'saved': return 'Database synced ✓';
...
export default CoachProgramsManagerFixed;