...
const WarmUpSection: React.FC<WarmUpSectionProps> = ({
  ...
  const [subsections, setSubsections] = useState<WarmUpSubsection[]>([]);
    ...
    try {
      ...
      const subsectionsWithExercises = await Promise.all(
        subsectionsData.map(async (subsection) => {
          ...
          // Set default sets based on subsection title
          let defaultSets = 1;
          ...
          return {
            ...
            repeat_count: subsection.repeat_count || defaultSets,
      ...
      console.error('Error loading warm-up subsections:', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      {subsections.map((subsection) => (
        <div key={subsection.id} className="mb-6">
          <CleanSubsectionHeader
            ...
            instruction={subsection.repeat_count > 1 ? `${subsection.repeat_count}x` : '1x'}
