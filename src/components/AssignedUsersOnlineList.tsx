...
interface AssignedUsersOnlineListProps {
  coachId: string;
  onUserSelect: (user: OnlineUser) => void;
  selectedUserId?: string;
}

export const AssignedUsersOnlineList: React.FC<AssignedUsersOnlineListProps> = ({
  coachId,
  onUserSelect,
  selectedUserId
}) => {
  ...
  useEffect(() => {
    if (coachId) {
      ...
      const channel = supabase
        ...
        .on(
          'postgres_changes',
          {
            ...
            schema: 'public',
            table: 'user_profiles',
            filter: `status=in.(online,away,busy)`
          },
          () => {
            fetchAssignedOnlineUsers();
          }
      ...
      const interval = setInterval(fetchAssignedOnlineUsers, 10000);
  ...
  const fetchAssignedOnlineUsers = async () => {
    try {
      // Get users assigned to this coach who are online
      const { data: assignments, error } = await supabase
        ...
        `)
         .eq('coach_id', coachId)
         .eq('is_active', true)
         .in('user_profiles.status', ['online', 'away', 'busy']);

      if (error) {
        console.error('Error fetching assigned online users:', error);
        return;
      }
      ...
      const users = assignments?.map(item => ({
        ...
        status: item.user_profiles.status,
        last_activity: item.user_profiles.last_activity
      })) || [];

      // Sort by status priority (online first, then away, then busy)
    ...
    return (
      <div className="p-4 text-center text-gray-500">
        <div className="flex flex-col items-center space-y-2">
          ...
          <p className="text-sm">No assigned users online</p>
        </div>
      </div>
    );
  }
