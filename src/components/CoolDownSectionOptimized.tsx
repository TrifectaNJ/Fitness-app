...
interface CoolDownSectionOptimizedProps {
  workoutId: string;
  currentExerciseIndex: number;
  allExercises: Exercise[];
  onExerciseClick: (exercise: Exercise, subsection?: any) => void;
  subsections: CoolDownSubsection[];
  userId: string;
}

const CoolDownSectionOptimized: React.FC<CoolDownSectionOptimizedProps> = ({
  workoutId,
  ...
  const handleExerciseClick = (exercise: Exercise, subsection: CoolDownSubsection) => {
    onExerciseClick(exercise, subsection);
  };

  const handleExerciseToggle = async (exerciseId: string, subsection: CoolDownSubsection) => {
              ...
              <div className="space-y-4">
                {subsection.exercises.map((exercise) => {
                  ...
                  return (
                    <div
                      key={exercise.id}
                      className={`group flex items-center p-4 rounded-xl border transition-all duration-300 cursor-pointer hover:shadow-md ${
                        isCurrent 
...
export default CoolDownSectionOptimized;